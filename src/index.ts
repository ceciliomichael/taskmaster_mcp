...
server.tool("save_memory", {
  projectPath: z.string().describe("Absolute path to the project directory"),
  content: z.string().describe("MANDATORY: You MUST use save_memory after completing ANY task, no matter how small or minimal. This is REQUIRED for ALL completed work. Store a very brief but very concise, clear, well-articulated and detailed summary that captures what was accomplished, what decisions were made, and what was learned. The summary should be comprehensive enough for future understanding while remaining concise. Never skip saving memory - even minor tasks must be documented for context continuity. IMPORTANT: Use save_memory only ONCE at the very END when the user's complete request is finished - NOT during intermediate steps, user only asks a question without task, investigations or when the user asks you to study the project, or when the user asks you to commit the project, or partial work. Wait until the entire user task is fully completed before saving.")
